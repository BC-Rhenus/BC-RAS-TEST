report 50034 "Consolidated Customs Invoice"

//  RHE-TNA 30-12-2020 BDS-4644
//  - Copy of report 50023

{
    // version NAVW18.00.00.39368

    DefaultLayout = RDLC;
    RDLCLayout = './Rhenus Consolidated Customs Invoice.rdl';
    Caption = 'Consolidated Customs Invoice';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING ("Document Type", "Importer of Record") WHERE ("Document Type" = CONST (Order));
            RequestFilterFields = "Ship-to Country/Region Code", "No.", "Sell-to Customer No.", "No. Printed";
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(BilltoCustNo_SalesHeader; IoRCustomer."No.")
                    {
                    }
                    ...
                    column(ShptDate_SalesHeader; Format(Today))
                    {
                    }
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        column(LineVATPerc; LineVatPerc)
                        {
                        ...
                        begin
                            ...
                            NNCSalesLineLineAmt += SalesLine."VAT Base Amount";
                            ...
                            if "Sales Header"."No." <> PrevOrder then begin
                                NNCVATAmt += VATAmount;
                                PrevOrder := "Sales Header"."No.";
                            end;
            ...
            begin
                ...
                if (Country.Get("Sales Header"."Ship-to Country/Region Code")) and (Country."Importer of Record" <> '') then
                    IoRCustomer.Get(Country."Importer of Record")
                else
                    IoRCustomer.Get("Sales Header"."Bill-to Customer No.");
                ...
                if IoRCustomer."No." <> PrevIoRCustomer."No." then begin
                    NNCTotalLCY := 0;
                    NNCTotalExclVAT := 0;
                    NNCVATAmt := 0;
                    NNCTotalInclVAT := 0;
                    ...
                    NNCSalesLineLineAmt := 0;
                    NNCSalesLineInvDiscAmt := 0;
                    PrevIoRCustomer.Get(IoRCustomer."No.");
                end else begin
                    //On first order CheckPrev = false
                    if CheckPrev then begin
                        if "Shipment Method Code" <> PrevShipmentMethod then
                            Error('Shipment Method Code does not match in all orders.');
                        if "Shipping Agent Code" <> PrevShippingAgent then
                            Error('Shipping Agent Code does not match in all orders.');
                    end else
                        CheckPrev := true;
                end;
                PrevShipmentMethod := "Shipment Method Code";
                PrevShippingAgent := "Shipping Agent Code";
            end;
            ...
            begin
                ...
                if CurrencyFilter <> '' then
                    "Sales Header".SetRange("Currency Code", CurrencyFilter)
                else
                    "Sales Header".SetFilter("Currency Code", '%1|%2', GLSetup."LCY Code", '');
            end;
        }
    }
    ...
    requestpage
    {
        ...
        layout
        {
            area(content)
            {
                group(Required)
                {
                    field("Currency Code"; CurrencyFilter)
                    {
    ...
    begin
        ...
        //Loop through orders to set Importer of Record
        SalesHdr.SetRange("Document Type", SalesHdr."Document Type"::Order);
        if SalesHdr.FindSet() then begin
            repeat
                if (Country.Get(SalesHdr."Ship-to Country/Region Code")) and (Country."Importer of Record" <> '') then
                    SalesHdr."Importer of Record" := Country."Importer of Record"
                else
                    SalesHdr."Importer of Record" := SalesHdr."Bill-to Customer No.";
    ...
    trigger OnPreReport()
    begin
        If CurrencyFilter = '' then begin
            if not Dialog.Confirm('No Currency Code filter is entered, Local Currency (' + GLSetup."LCY Code" + ') will be used. Do you want to continue?') then
                CurrReport.Quit();
        end;
    end;
    ...
    var
        ...
        Text003: Label 'COPY';
        Text004: Label 'Consolidated Customs Invoice%1';
        ...
        PrevIoRCustomer: Record Customer;
        PrevShippingAgent: Code[10];
        PrevShipmentMethod: Code[10];
        CheckPrev: Boolean;
